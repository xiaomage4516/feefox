...
cc.Class({
    ...
    initScene : function (type) {
        ...
        if(gameData.matchLevelData_houduan){//后端返回的表数据，  底分由后端控制
            ...
            let list_h = [];
            var matct_str = JSON.stringify(gameData.matchLevelData_houduan);
            var match = JSON.parse(matct_str);
            for(let i = 0 ; i < match.length; i++){
                if (match[i].type === ""+that.gameType) {
                    list_h.push(match[i]);
                }
            }
        ...
        var callback = function (jsonData) {
            ...
            }else if(that.gameType+"" === "100"){
                people.push(gameData.pdkLevel1NUm);
                people.push(gameData.pdkLevel2NUm);
                people.push(gameData.pdkLevel3NUm);
                people.push(gameData.pdkLevel4NUm);
                people.push(gameData.pdkLevel5NUm);
                people.push(gameData.pdkLevel6NUm);
            ...
            for(let i = 0 ; i < list.length; i++){ 
                ...
                if (bg) {
                    util.loadSprite("hall/more_field/bg/"+data.tab_id, bg.getChildByName("bg"));
                    // util.loadSprite("hall/more_field/title/"+data.tab_id, bg.getChildByName("title"));
                    // bg.getChildByName("difen").getComponent(cc.Label).string =  data.serveprice;//改成后端控制
                    bg.getChildByName("room_people_num").getComponent(cc.Label).string = people[i];
                    bg.getChildByName("coin_num").getComponent(cc.Label).string =  util.showNumber(data.limmitdown)+"—"+ util.showNumber(data.limmitup);
                }
            }
    ...
    onBtnSelGameLevel:function(){
        ...
        util.getPlayTypeStatus({
            ...
            tipCallBack : function(){
                ...
                // });
                //充值提示
                var nowSelLevel = self._nowSelLevel;
                if(nowSelLevel>3){
                    nowSelLevel -= 3;
                }
                util.tip4({
    ...
    onBtnQuickStart:function(){
        ...
        util.getPlayTypeStatus({
            ...
            fieldId : 1,
            callBack : function(data){
                gameData.mateLevel = data.quickGameLevel;
                // 加入匹配场
                var info = {
                    playerId: gameData.uid,
                    mapId: self.gameType,//玩法ID 现在只做了两个，暂时写死
